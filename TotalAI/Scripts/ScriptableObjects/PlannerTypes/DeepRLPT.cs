...
{
    [CreateAssetMenu(fileName = "DeepRLPT", menuName = "Total AI/Planner Types/Deep RL", order = 0)]
    public class DeepRLPT : PlannerType
    {
        ...
        {
            Debug.LogError("DeepRLPT.ReevaluateTargets not implememted.  There is no chaining of Mappings so this is not needed.");
        }

        // Drive Type is ignored by Deep RL - It always returns all possible mappings
        ...
        private Plans PossiblePlans(Agent agent, DriveType driveType)
        {
            ...
            foreach (MappingType mappingType in agent.availableMappingTypes)
            {
                ...
                // TODO: Return all targets each one as a seperate Mapping - so Deep RL can decide between them
        ...
        private bool SetInventoryTargets(Agent agent, Mapping mapping, InputCondition inputCondition)
        {
            ...
            {
                Debug.Log(agent.name + ": DeepRLPT.BestInventoryTargets unable to find target - MT = " + mapping.mappingType);
                return false;
            }
        ...
        private List<KeyValuePair<Entity, float>> SelectTarget(Agent agent, Mapping mapping, List<Entity> entities, bool useInventoryTFs)
        {
            ...
            foreach (Entity entity in entities)
            {
                ...
                {
                    Debug.LogError(agent.name + ": DeepRLPT - MappingType (" + mapping.mappingType.name + ") has no " + (useInventoryTFs ? "Inventory" : "Target") +
                                   " Factors.  Please Fix.");
                    return null;
                }
